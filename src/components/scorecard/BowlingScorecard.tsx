  ...
  return (
    <div className='shadow-md rounded-md border border-gray-500/20 dark:bg-gray-600/10 p-4 mb-6'>
        <div className='flex font-bold justify-end mb-3'>
            {['o', 'm', 'r', 'w', 'e'].map((header) => (
                <div key={header} className={`w-14 text-right ${header === 'e' ? 'pr-3' : ''}`}>
                    {header.toUpperCase()}
                </div>
            ))}
        </div>
      ...
      {inn.bowling.map(b => (
        <div
          key={b.bowler}
          className='flex justify-between border-b last:border-b-0 border-gray-600/20 py-1 text-sm'
        >
